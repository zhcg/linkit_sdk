...
int wfcm_if_sdio_init(void)
{
    ...
    /*[JCB]need move it from wfcm_if_sdio_init(),  to avoid g_sem_qbuf be create duplicate,   
          as the wfcm_if_sdio_init() be called many times */
...
bool wfcm_sdio_send_data(T_WFC_QBUF *qbuf, uint8_t qbuf_release)
{
    ...
    if (tx_que != 0) {
        tx_header->u.bits.tx_type = tx_que-1;
    }
