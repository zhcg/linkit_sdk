...
#ifdef MTK_WIFI_STUB_CONF_ENABLE
            uint32_t size = sizeof(wh_scan_reslt_evt_para_t) + sizeof(wifi_scan_list_item_t)*scan_ap_size;
            wh_scan_reslt_evt_para_t *evt_para = (wh_scan_reslt_evt_para_t *)os_malloc(size);
...
int32_t wfc_api_process(uint8_t *pcmd,  uint32_t cmdSize, uint8_t *txptr, uint32_t *txsize)
{
    ...
    {                 
        ...
        {
            if(api_id->set == true) {   //JCB Test (enable below codes)
...
int32_t wfc_set_wifi_host_mac(uint8_t port, uint8_t *pmac_addr)
{
    if (port == WIFI_PORT_STA){
        memcpy(g_wfc_system_config.wfc_wifi_host_adv_config.sta_mac, pmac_addr, WIFI_MAC_ADDRESS_LENGTH);
    }else if(port == WIFI_PORT_AP){
        memcpy(g_wfc_system_config.wfc_wifi_host_adv_config.ap_mac, pmac_addr, WIFI_MAC_ADDRESS_LENGTH);
    }
    return 0;
}
...
{   
    memcpy(&g_wfc_system_config.wfc_wifi_config, pconfig, sizeof(wifi_config_t) );

    memcpy(&g_wfc_system_config.wfc_wifi_config_ext, pconfig_ext, sizeof(wifi_config_ext_t));
...
void wfc_get_wifi_host_setting(wifi_config_t *pconfig, wifi_config_ext_t *pconfig_ext)
{
    ...
    if( g_sem_wfc_host_cfg != NULL ) {
        if (wifi_os_semphr_take(g_sem_wfc_host_cfg, NULL) == pdTRUE){

            /*2: copy the config ,config_ext setting from wfc module*/
            memcpy(pconfig, &g_wfc_system_config.wfc_wifi_config,sizeof(wifi_config_t) );
            memcpy(pconfig_ext, &g_wfc_system_config.wfc_wifi_config_ext, sizeof(wifi_config_ext_t) );
