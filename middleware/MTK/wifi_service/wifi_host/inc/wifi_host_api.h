...
* #wifi_host_config_set_pmk() \n
* #wifi_host_config_set_security_mode() \n
* #wifi_host_config_set_ssid() \n
* #wifi_host_config_set_wep_key() \n
* #wifi_host_config_set_wpa_psk_key() \n
* The Configuration APIs include wifi_host_config_xxx APIs and wifi_host_wps_config_xxx APIs. They use in-band mechanism (in-band is a mechanism used for Wi-Fi driver and Wi-Fi firmware communication),
* thus these APIs must be called after OS task scheduler is started to make sure an in-band task is running.
* The wifi_host_wps_config_xxx APIs can be used to set or get default device configuration before triggering the Push Button Connection (PBC) or Personal Identification Number (PIN) process.
* The function #wifi_host_config_register_rx_handler() registers wifi_rx_handler to manage the raw packet. There is a limitation calling this handler as it's using an in-band mechanism,
* it's restricted to call any in-band function similar to Wi-Fi Configuration or Wi-Fi Connection APIs in wifi_rx_handler.
*
* The Connection APIs include wifi_host_connection_xxx APIs and wifi_host_wps_connection_xxx APIs. They manage the link status, such as disconnect AP, disconnect the station, get the link status, get the station list, start or stop the scan.
...
* The wifi_host_wps_connection_xxx APIs can be used to trigger the PBC or PIN process.
*
*
* Terms                         |Details                                                                 |
* ------------------------------|------------------------------------------------------------------------|
