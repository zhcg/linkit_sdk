 ...
 *
 *           status = nvdm_init();
 *           status = nvdm_write_data_item("network", "IpAddr", DATA_ITEM_TYPE_STRING, (uint8_t *)STA_IPADDR_DEFAULT_VALUE, sizeof(STA_IPADDR_DEFAULT_VALUE));
 *           size = sizeof(buffer);
 *           status = nvdm_read_data_item("network", "IpAddr", buffer, &size);
 *       }
 *    @endcode
 *
