...
amr_decoder_handle_t *amr_decoder_open(amr_decoder_callback_t amr_decoder_callback)
{
    ...
    internal_handle->stream_out_pcm_buff.read_pointer = 0;
    internal_handle->stream_out_pcm_buff.write_pointer = 0;
    internal_handle->stream_out_pcm_buff.buffer_base_pointer  = (uint8_t *)pvPortMalloc(internal_handle->stream_out_pcm_buff_size * sizeof(uint8_t));
    ...
    internal_handle->dec_int_buffer = (uint8_t *)pvPortMalloc(internal_handle->dec_int_buf_size * sizeof(uint8_t));
    if (internal_handle->dec_int_buffer == NULL) {
        LOGE("pvPortMalloc dec_int_buffer failed\r\n");
        return NULL;
    }

    internal_handle->dec_tmp_buffer = (uint8_t *)pvPortMalloc(internal_handle->dec_tmp_buf_size * sizeof(uint8_t));
    if (internal_handle->dec_tmp_buffer == NULL) {
        LOGE("pvPortMalloc dec_tmp_buffer failed\r\n");
        return NULL;
    }

    internal_handle->dec_pcm_buffer = (int16_t *)pvPortMalloc(internal_handle->dec_pcm_buf_size * sizeof(uint8_t));
    if (internal_handle->dec_pcm_buffer == NULL) {
        LOGE("pvPortMalloc dec_pcm_buffer failed\r\n");
        return NULL;
    }

    internal_handle->dec_bs_buffer  = (uint8_t *)pvPortMalloc(internal_handle->dec_bs_buf_size * sizeof(uint8_t));
