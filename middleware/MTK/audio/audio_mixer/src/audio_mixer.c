...
static void audio_mixer_adapt_resample_open( audio_mixer_track_id_t track_id)
{
...
static void audio_mixer_activate_track(
    audio_mixer_track_id_t          track_id)
{
    audio_mixer_adapt_resample_open(track_id);
...
static void audio_mixer_stream_out_callback(hal_audio_event_t event, void *data)
{
    ...
    // KH TODO: should calculate available data, then compare with DSP space
...
static void audio_mixer_start_HAL(
    audio_mixer_track_id_t          track_id)
{
    ...
    // Activate track.
    audio_mixer_activate_track(track_id);
    ...
    audio_mixer_track_id_t          track_id)
{
    ...
    if (total_active_track == 0) {
        audio_mixer_start_HAL(track_id);
    } else {
    ...
    audio_mixer_track_id_t          track_id)
{
    ...
    if (Audio_Mixer.track[track_id].state != AUDIO_MIXER_STATE_MACHINE_RUNNING) {
        log_hal_error("[Mixer] stopt: track %d wrong state\n", track_id);
        return AUDIO_MIXER_STATUS_ERROR;
    }
...
//    log_hal_info("[Mixer] track %d, size %d\n", track_id, size);
    ...
    audio_mixer_query_free_count(track_id, &free_count);
