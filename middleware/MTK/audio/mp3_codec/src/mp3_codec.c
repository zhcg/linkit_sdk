...
static void mp3_codec_mutex_lock(mp3_codec_media_handle_t *handle)
{
    ...
    if (internal_handle->semaphore_handle != NULL) {
        log_hal_info("[MP3 Codec] mp3_codec_mutex_lock() +\r\n");
...
static void mp3_codec_mutex_unlock(mp3_codec_media_handle_t *handle)
{
    ...
    if (internal_handle->semaphore_handle != NULL) {
        log_hal_info("[MP3 Codec] mp3_codec_mutex_unlock()\r\n");
...
static void mp3_codec_flush(mp3_codec_media_handle_t *handle, int32_t flush_data_flag)
{
    ...
    log_hal_info("[MP3 Codec] flush_data_flag = %d\n", handle->flush_data_flag);
}
...
static int32_t mp3_codec_set_silence_frame_information(mp3_codec_media_handle_t *handle, silence_frame_information_t *frm_info)
{
    if (frm_info->frame_size > 0) {
        ...
        log_hal_info("MP3SilenceFrameSize = %d, MP3SilenceFrameHeader = 0x%X %X %X %X \n", MP3SilenceFrameSize, 
...
mp3_codec_function_return_state_t mp3_codec_aws_init(mp3_codec_media_handle_t *handle)
{
    ...
    log_hal_info("mp3_codec_aws_init: p_aws_buf = 0x%X, aws_buf_size = %d\n", p_aws_buf, aws_buf_size);
    if (p_aws_buf) {
        aws_buf_size = audio_service_aws_init(p_aws_buf, AWS_CODEC_TYPE_PCM_FORMAT, mp3_codec_aws_callback, (void *)handle);
        log_hal_info("audio_service_aws_init mp3_codec_aws_callback(0x%X)\n", mp3_codec_aws_callback);
        log_hal_info("audio_service_aws_init return %d\n", aws_buf_size);
    } else {
...
{
    log_hal_info("mp3_codec_aws_deinit: Free p_aws_buf\r\n");
...
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] role%d request data again: share = %d, pcm = %d\r\n", handle->mixer_track_role, mp3_codec_get_share_buffer_data_count(handle), ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff));
#else
        log_hal_info("[MP3 Codec] request data again: share = %d, pcm = %d\r\n", mp3_codec_get_share_buffer_data_count(handle), ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff));
#endif
        return MP3_CODEC_RETURN_ERROR;
    }
}
...
#endif
{
    ...
    if (NULL == mp3_handle_ptr_to_mp3_decode_buffer(handle)) {
        log_hal_error("[MP3 Codec] mp3_codec_set_memory2 fail: cannot allocate mp3 decode buffer\n");
        return MP3_CODEC_RETURN_ERROR;
    }
...
#endif // #ifndef MTK_AUDIO_MIXER_SUPPORT
...
static void mp3_codec_pcm_out_isr_callback(hal_audio_event_t event, void *data)
{
    ...
    if (1 == internal_handle->media_bitstream_end) {
        log_hal_info("[MP3 Codec] isr just return becase media_bitstream_end == 1\r\n");
#if defined(MTK_AUDIO_MIXER_SUPPORT)
#else
        ...
        case HAL_AUDIO_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 2; loop_idx++) {
                ...
                if (stream_out_pcm_buffer_data_count > 0) {
                    ...
                    uint32_t curr_cnt = 0;
                    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &curr_cnt);
                    log_hal_info("[MP3 Codec] curr_cnt=%d, stream_out_pcm_buff no data. loop_idx=%d\r\n", (curr_cnt / 1000), loop_idx);
#if defined(MTK_AUDIO_MIXER_SUPPORT)
            ...
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &cnt1);

            int32_t i = 0;
            for(i = 0; i < 14; i++) { // 70 ms = 5 * 14 // 14.22 = 16384/(576*2)
                ...
                if (ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff) < one_frame_bytes) {
                    log_hal_info("[MP3 Codec] pcm space(%d) < one_frame_bytes(%d), just return\r\n", ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff), one_frame_bytes);
                    break;
                }

                if (ring_buffer_get_data_byte_count(&internal_handle->stream_out_pcm_buff) >= mp3_handle_ptr_to_total_sample_count(handle)) {
                    log_hal_info("[MP3 Codec] sample_count = %d Full\r\n", mp3_handle_ptr_to_total_sample_count(handle));
                    break;
                }
                ...
                if (mp3_codec_get_share_buffer_data_count(handle) < check_min_share_size) {
                    mp3_codec_request_data_to_share_buffer(handle);
                    break;
                }

                mp3_codec_deocde_hisr_handler(handle);
            }
            hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &cnt2);
            // log_hal_info("[MP3 Codec] role%d, i = %d, pcm_buff_data_byte_count = %d, cnt = %d\r\n", handle->mixer_track_role, i, ring_buffer_get_data_byte_count(&internal_handle->stream_out_pcm_buff), (cnt2 - cnt1) / 1000);
#endif
#if defined(MTK_AUDIO_MIXER_SUPPORT)
            if ((handle->linear_buff == 1) && (handle->share_buff.read == handle->share_buff.buffer_size)) {
                int32_t data_status = 0;        
                mp3_codec_get_data_status(handle, MP3_CODEC_DATA_TYPE_AUDIO, &data_status);
                if ((data_status == 1) && (mp3_handle_ptr_to_is_bitstream_end_called(handle) == 0)) {
                    // handle->handler(handle, MP3_CODEC_MEDIA_BITSTREAM_END);
                    audio_mixer_set_eof(handle->mixer_track_id);
                    mp3_handle_ptr_to_is_bitstream_end_called(handle) = 1;
                }            
            }

            if ((handle->flush_data_flag == 1) && (mp3_handle_ptr_to_is_bitstream_end_called(handle) == 0)) {
                int32_t share_buffer_data_amount = mp3_codec_get_share_buffer_data_count(handle);
                if (share_buffer_data_amount < MPEG_AUDIO_FRAME_HEADER_LENGTH) {
                    int32_t data_status = 0;
                    mp3_codec_get_data_status(handle, MP3_CODEC_DATA_TYPE_AUDIO, &data_status);
        ...
        case HAL_AUDIO_EVENT_END:
            {
                uint32_t curr_cnt = 0;
                hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &curr_cnt);
                log_hal_info("[MP3 Codec] isr(%d), MP3_CODEC_MEDIA_BITSTREAM_END\r\n", (curr_cnt / 1000));
            }
...
static int32_t mp3_codec_get_next_mp3_frame_size(uint8_t *mp3_header, uint8_t *buffer_base, int32_t buffer_size, int32_t *mp3_frame_size)
{
    ...
    *mp3_frame_size = 0;
    ...
    if (IS_MP3_HEAD(temp_mp3_header)) {
        ...
        // log_hal_info("[MP3 Codec] frameSize = %d, bitrate = %d, samplingRate = %d, paddingBit = %d\n", frameSize, bitrate, samplingRate, paddingBit);
        if (frameSize > buffer_size) {
            *mp3_frame_size = 0;
            log_hal_info("[MP3 Codec] Set frameSize to 0 because frameSize(%d) > buffer_size(%d)\n", frameSize, buffer_size);
        }
        ...
        // log_hal_info("[MP3 Codec] The next frame header is 0x%04X\n", temp_mp3_header);
        *mp3_frame_size = frameSize;
        if (IS_MP3_HEAD(temp_mp3_header)) {
            // log_hal_info("[MP3 Codec] bitrate = %d, samplingRate = %d, paddingBit = %d, frameSize = %d\n", bitrate, samplingRate, paddingBit, frameSize);
...
static void mp3_codec_deocde_hisr_handler(void *data)
{
    ...
    if (1 == internal_handle->media_bitstream_end) {
        log_hal_info("[MP3 Codec] hisr(%d): handle->handler(handle, MP3_CODEC_MEDIA_BITSTREAM_END);\r\n", (curr_cnt / 1000));
        handle->handler(handle, MP3_CODEC_MEDIA_BITSTREAM_END);
        return;
    }
    ...
    if ((1 == handle->flush_data_flag) && (share_buffer_data_amount <= MPEG_AUDIO_FRAME_HEADER_LENGTH)) {
        ...
        if (0 == stream_out_pcm_buffer_data_count) {
            log_hal_info("[MP3 Codec] hisr(%d): audio_pcm_set_eof()\r\n", (curr_cnt / 1000));
            audio_pcm_set_eof();
        }
    }
#endif
    ...
    if (share_buffer_data_amount < (check_min_share_size + 1)) {
        ...
        } while ((share_buffer_data_amount < check_min_share_size) && (share_buffer_too_less_loop_times < SHARE_BUFFER_TOO_LESS_SLEEP_LOOP_TIMES));
#endif
        if (mp3_codec_get_share_buffer_data_count(handle) < (check_min_share_size + 1)) {
            mp3_codec_request_data_to_share_buffer(handle);
            return;
        }
    }
...
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("hisr(%d): role%d: mp3 frame[ 0] = 0x%02X %02X %02X %02X\r\n",
    ...
    if (consumeBS < 0) {
        ...
        if (consumeBS == 1) {
#if defined(MTK_AUDIO_MIXER_SUPPORT)
            log_hal_info("[MP3 Codec] hisr(%d): role%d: decode abnormal. consumeBS =1, handle->share_buff.read = %d, handle->share_buff.write = %d, handle->share_buff.buffer_size = %d\r\n", (curr_cnt / 1000), handle->mixer_track_role, handle->share_buff.read, handle->share_buff.write, handle->share_buff.buffer_size);
#else
            log_hal_info("[MP3 Codec] hisr(%d): decode abnormal. consumeBS =1, handle->share_buff.read = %d, handle->share_buff.write = %d, handle->share_buff.buffer_size = %d\r\n", (curr_cnt / 1000), handle->share_buff.read, handle->share_buff.write, handle->share_buff.buffer_size);
#endif
...
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] hisr(%d): role%d, decode abnormal. consumeBS > share_data_amount\r\n", (curr_cnt / 1000), handle->mixer_track_role);
#else
...
#if defined(MTK_AUDIO_MIXER_SUPPORT)
    if (mp3_codec_get_share_buffer_data_count(handle) < handle->share_buff.buffer_size) {
#else
...
/* temp sol. to protect play/resume flow  */
    ...
    do {
        ...
        if (consumeBS < 0) {
            ...
            log_hal_info("[MP3 Codec] MP3SilenceFrameSize = %d, MP3SilenceFrameHeader = 0x%02X %02X %02X %02X \n", MP3SilenceFrameSize,
                MP3SilenceFrameHeader[0], MP3SilenceFrameHeader[1], MP3SilenceFrameHeader[2], MP3SilenceFrameHeader[3]);
        }
        ...
        if (max_loop_times < 0) {
            log_hal_info("[MP3 Codec] mp3_codec_play_internal: decode mp3 data, but fail, please check the file\n");
            return MP3_CODEC_RETURN_ERROR;
        }
    ...
    log_hal_info("[MP3 Codec] start mixer: %d, handle(%X) role%d\n", audio_mixer_status, handle, handle->mixer_track_role);
...
{    
    log_hal_info("[MP3 Codec] resume ++\r\n");
    audio_codec_mutex_lock();
    mp3_codec_mutex_lock(handle);
    if (handle->state != MP3_CODEC_STATE_PAUSE) {
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] resume --: handle(%X) role%d, cannot resume because state(%d)\r\n", handle, handle->mixer_track_role, handle->state);
#else
        log_hal_info("[MP3 Codec] resume --: cannot resume because state(%d)\r\n", handle->state);
#endif
    ...
    mp3_handle_ptr_to_pcm_isr_cnt(handle) = 0;
    audio_mixer_start_track(handle->mixer_track_id);
#else
    hal_audio_start_stream_out(HAL_AUDIO_PLAYBACK_MUSIC);
#endif
    log_hal_info("[MP3 Codec] resume: stream out pcm data=%d, share buffer data=%d\r\n", ring_buffer_get_data_byte_count(&(mp3_handle_ptr_to_internal_ptr(handle))->stream_out_pcm_buff), mp3_codec_get_share_buffer_data_count(handle));
...
{
    log_hal_info("[MP3 Codec] pause ++\r\n");
    mp3_codec_mutex_lock(handle);
    if (handle->state != MP3_CODEC_STATE_PLAY) {
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] pause --: handle(%X) role%d, cannot pause because state(%d)\r\n", handle, handle->mixer_track_role, handle->state);
#else
        log_hal_info("[MP3 Codec] pause --: cannot pause because state(%d)\r\n", handle->state);
#endif
        mp3_codec_mutex_unlock(handle);
        return MP3_CODEC_RETURN_ERROR;
    }
...
{
    log_hal_info("[MP3 Codec] mp3_codec_play ++\r\n");
...
{
#if defined(MTK_AUDIO_MIXER_SUPPORT)
    log_hal_info("[MP3 Codec] stop ++: handle(%X) role%d\r\n", handle, handle->mixer_track_role);
#else
    log_hal_info("[MP3 Codec] stop ++\r\n");
#endif
    mp3_codec_mutex_lock(handle);
    if (handle->state != MP3_CODEC_STATE_PLAY && handle->state != MP3_CODEC_STATE_PAUSE) {
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] stop --: handle(%X) role%d, cannot stop because state(%d)\r\n", handle, handle->mixer_track_role, handle->state);
#else
        log_hal_info("[MP3 Codec] stop --: cannot stop because state(%d)\r\n", handle->state);
#endif
        mp3_codec_mutex_unlock(handle);
        return MP3_CODEC_RETURN_ERROR;
    }
...
{
#if defined(MTK_AUDIO_MIXER_SUPPORT)
    log_hal_info("[MP3 Codec] close++ handle(%X), role%d\n", handle, handle->mixer_track_role);
#else
    log_hal_info("[MP3 Codec] close++\n");
#endif
    mp3_codec_mutex_lock(handle);
    mp3_codec_internal_handle_t *internal_handle = mp3_handle_ptr_to_internal_ptr(handle);
    ...
    if (handle->state != MP3_CODEC_STATE_STOP && handle->state != MP3_CODEC_STATE_READY) {
        mp3_codec_mutex_unlock(handle);
#if defined(MTK_AUDIO_MIXER_SUPPORT)
        log_hal_info("[MP3 Codec] close--: handle(%X) role%d, cannot close because state(%d)\r\n", handle, handle->mixer_track_role, handle->state);
#else
        log_hal_info("[MP3 Codec] close--: cannot close because state(%d)\r\n", handle->state);
#endif
        return MP3_CODEC_RETURN_ERROR;
    }
...
mp3_codec_media_handle_t *mp3_codec_open(mp3_codec_callback_t mp3_codec_callback)
{
    ...
    if (NULL == private_data) {        
        log_hal_error("[MP3 Codec] mp3_codec_open failed: cannot allocate private_data\n");
        return NULL;
    }
    ...
    if (NULL == internal_handle) {
        log_hal_error("[MP3 Codec] mp3_codec_open failed: cannot allocate internal_handle\n");
        return NULL;
    }
    memset(internal_handle, 0, sizeof(mp3_codec_internal_handle_t));
    log_hal_info("mp3_codec_open: internal_handle = 0x%X", internal_handle);
    ...
    if (private_data->mp3_codec_queue_hdl == NULL) {
        ...
        log_hal_info("[MP3 Codec] create mp3 queue (0x%X)\r\n", private_data->mp3_codec_queue_hdl);
    }
    ...
    if (blIsRegister == false) {
        blIsRegister = true;
        hal_cm4_topsm_register_resume_cb((cm4_topsm_cb)mp3_codec_enter_resume, handle);
        hal_cm4_topsm_register_suspend_cb((cm4_topsm_cb)mp3_codec_enter_suspend, handle);
    }
...
void mp3_codec_enter_resume(void *data)
{
    ...
    if (mp3_handle_ptr_to_internal_ptr(data)->IsMP3Exit == false) {
        return ;
    }
