...
/* For reach the first frame of MP3, some MP3 may have other data between ID3 header and frame */
    {
        ...
        frame_len = bt_sink_srv_mp3_search_mp3_header(temp_frame_buff, AUIDO_SKIP_ID3_FILE_LENGTH, &frame_start);
...
void bt_sink_srv_aws_mp3_gpt_cb(void *user_data)
{
    ...
    play_clock = dev->play_time.nclk * 312 +
...
static int32_t bt_sink_srv_mp3_aws_send_streaming_data(uint8_t *data, int32_t total_len)
{
    ...
    if (remain_len != 0) {
        bt_sink_srv_mp3_update_frame_info(&data, &remain_len);
        /* Find context not belong to mp3 header, switch to next track */
        if (ret < 0) {
            return -3;
        }
...
static void bt_sink_srv_mp3_local_play(audio_src_srv_handle_t *handle)
{
    ...
    if (mp3_ctx->file_total > 0
        ...
        TCHAR new_file_path[MAX_FILE_PATH_LEN] = {0};
        if (mp3_ctx->offset > 0) {
            ...
            if (!(dev->flag & BT_SINK_SRV_MUSIC_FLAG_MP3_FILE_OPEN)) {
                ...
                event.get_file_path_by_idx.path = new_file_path;
                ...
                memcpy(mp3_ctx->curr_file_path, new_file_path, sizeof(new_file_path));
                bt_sink_srv_report("[MP3]bt_sink_srv_mp3_local_play(1)--idx: %d, name: %s",
                                                                                mp3_ctx->file_index, new_file_path);
                ret = bt_sink_srv_mp3_open_file(&g_aud_fdst, mp3_ctx->curr_file_path);
                type = get_file_type();
                if ( type == FILE_MP3)
                    if (mp3_ctx->offset >= bt_sink_srv_mp3_size_file(&g_aud_fdst)) {
                        mp3_ctx->offset = 0;
                    }
            ...
            memcpy(mp3_ctx->curr_file_path, new_file_path, sizeof(new_file_path));
            bt_sink_srv_report("[MP3]bt_sink_srv_mp3_local_play(2)--idx: %d, name: %s", mp3_ctx->file_index, new_file_path);
            ret = bt_sink_srv_mp3_open_file(&g_aud_fdst, mp3_ctx->curr_file_path);
        ...
        if ( type == FILE_MP3 ) {
            ...
            if (result) {
                ...
                bt_sink_srv_assert(0 && "get mp3 header fail");
            }
        }
    } else {
...
void bt_sink_srv_mp3_init(void)
{
    ...
    /* default setting */
    g_aud_cntx.vol_lev = (uint8_t)BT_SINK_SRV_MP3_DEFAULT_VOLUME;
