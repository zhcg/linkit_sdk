...
uint32_t pre_wakeup_systick = 0;
uint32_t cur_wakeup_systick = 0;
...
void AST_vPortSuppressTicksAndSleep(TickType_t xExpectedIdleTime)
{
    ...
    if (eTaskConfirmSleepModeStatus() == eAbortSleep) {
        ...
        cur_wakeup_systick = nowCount;
        if ((sleepdrv_get_sleep_mode() != HAL_SLEEP_MODE_SLEEP) && 
            ((cur_wakeup_systick - pre_wakeup_systick) < 100)) { // less than about 3ms
            busy_wakeup_num++;
        } else {
            busy_wakeup_num = 0;
        }
        pre_wakeup_systick = cur_wakeup_systick;
#endif
