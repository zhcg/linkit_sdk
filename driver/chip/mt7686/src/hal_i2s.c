...
static hal_clock_cg_id i2s_clock_cg[HAL_I2S_MAX];
static vdma_channel_t i2s_vdma_tx_ch[HAL_I2S_MAX];
static vdma_channel_t i2s_vdma_rx_ch[HAL_I2S_MAX];
...
hal_i2s_status_t hal_i2s_init_ex(hal_i2s_port_t i2s_port, hal_i2s_initial_type_t i2s_initial_type)
{
    ...
    //Initialize variable
    //I2S0
    i2s_clock_cg[HAL_I2S_0] = HAL_CLOCK_CG_SW_AUDIO;
    i2s_vdma_tx_ch[HAL_I2S_0] = VDMA_AUDTX;
    i2s_vdma_rx_ch[HAL_I2S_0] = VDMA_AUDRX;

    //I2S1
    i2s_clock_cg[HAL_I2S_1] = HAL_CLOCK_CG_SW_ASYS;
    i2s_vdma_tx_ch[HAL_I2S_1] = VDMA_ASYSTX;
    i2s_vdma_rx_ch[HAL_I2S_1] = VDMA_ASYSRX;
...
hal_i2s_status_t hal_i2s_set_config_ex(hal_i2s_port_t i2s_port, const hal_i2s_config_t *config)
{
    ...
    if ((i2s_internal_cfg[HAL_I2S_0].i2s_state == I2S_STATE_INIT) &&
            (i2s_internal_cfg[HAL_I2S_1].i2s_state == I2S_STATE_INIT) &&
            (i2s_internal_cfg[HAL_I2S_0].i2s_clock_source != HAL_I2S_CLOCK_MAX) &&
            (i2s_internal_cfg[HAL_I2S_1].i2s_clock_source != HAL_I2S_CLOCK_MAX)) {
...
{
    vdma_status_t i2s_vdma_status[HAL_I2S_MAX] = {VDMA_OK};
    vdma_config_t i2s_vdma_config[HAL_I2S_MAX];
    uint32_t i2s_threshold[HAL_I2S_MAX] = {0};
    uint32_t irq_status;
    ...
    i2s_vdma_config[i2s_port].base_address = (uint32_t)buffer;
    i2s_vdma_config[i2s_port].size = buffer_length;
    i2s_threshold[i2s_port] = threshold;
    restore_interrupt_mask(irq_status);

    i2s_vdma_status[i2s_port] = vdma_init(i2s_vdma_tx_ch[i2s_port]);
    if (i2s_vdma_status[i2s_port]  != VDMA_OK) {
        log_hal_error("[I2S%d] SET TX VFIFO ERROR, vdma_init fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_configure(i2s_vdma_tx_ch[i2s_port], &i2s_vdma_config[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET TX VFIFO ERROR, vdma_configure fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_set_threshold(i2s_vdma_tx_ch[i2s_port], i2s_threshold[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET TX VFIFO ERROR, vdma_set_threshold fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_start(i2s_vdma_tx_ch[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET TX VFIFO ERROR, vdma_start fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }
    i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_base = (uint32_t *)i2s_vdma_config[i2s_port].base_address;
    i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_length = i2s_vdma_config[i2s_port].size;
    i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_threshold = i2s_threshold[i2s_port];
    i2s_internal_cfg[i2s_port].i2s_vfifo.tx_dma_configured = true;
...
hal_i2s_status_t hal_i2s_setup_rx_vfifo_ex(hal_i2s_port_t i2s_port, uint32_t *buffer, uint32_t threshold, uint32_t buffer_length)
{
    ...
    i2s_vdma_status[i2s_port] = vdma_init(i2s_vdma_rx_ch[i2s_port]);
    if (i2s_vdma_status[i2s_port]  != VDMA_OK) {
        log_hal_error("[I2S%d] SET RX VFIFO ERROR, vdma_init fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_configure(i2s_vdma_rx_ch[i2s_port], &i2s_vdma_config[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET RX VFIFO ERROR, vdma_configure fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_set_threshold(i2s_vdma_rx_ch[i2s_port], i2s_threshold[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET RX VFIFO ERROR, vdma_set_threshold fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }

    i2s_vdma_status[i2s_port] = vdma_start(i2s_vdma_rx_ch[i2s_port]);
    if (i2s_vdma_status[i2s_port] != VDMA_OK) {
        log_hal_error("[I2S%d] SET RX VFIFO ERROR, vdma_start fail\r\n", i2s_port);
        return HAL_I2S_STATUS_ERROR;
    }
    i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_base = (uint32_t *)i2s_vdma_config[i2s_port].base_address;
    i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_length = i2s_vdma_config[i2s_port].size;
    i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_threshold = i2s_threshold[i2s_port];
...
{
    return hal_i2s_stop_tx_vfifo_ex(HAL_I2S_0);
}
...
{
    if (i2s_port != HAL_I2S_0 && i2s_port != HAL_I2S_1) {
        log_hal_error("[I2S%d] STOP TX VFIFO ERROR, invalid i2s_port\r\n", i2s_port);
        return HAL_I2S_STATUS_INVALID_PARAMETER;
    }
...
{
    return hal_i2s_stop_rx_vfifo_ex(HAL_I2S_0);
}
...
{
    if (i2s_port != HAL_I2S_0 && i2s_port != HAL_I2S_1) {
        log_hal_error("[I2S%d] STOP RX VFIFO ERROR, invalid i2s_port\r\n", i2s_port);
        return HAL_I2S_STATUS_INVALID_PARAMETER;
    }
...
hal_i2s_status_t  hal_i2s_enable_tx_ex(hal_i2s_port_t i2s_port)
{
    ...
    if (i2s_port == HAL_I2S_0) {
        i2s_internal_cfg[HAL_I2S_0].i2s_state |= (uint8_t)(1 << I2S_STATE_TX_RUNNING);
        DRV_Reg8(I2S_DL_SR_EN_CONTROL__F_CR_I2S_OUT_EN) = (uint8_t)0x1;
    } else {
        i2s_internal_cfg[HAL_I2S_1].i2s_state |= (uint8_t)(1 << I2S_STATE_TX_RUNNING);
...
hal_i2s_status_t  hal_i2s_disable_tx_ex(hal_i2s_port_t i2s_port)
{
    ...
    if (i2s_port == HAL_I2S_0) {
        ...
        //log_hal_info("i2s0_state=%d\r\n", i2s0_internal_cfg.i2s_state);
        ...
        i2s_query_tx_vfifo_empty(i2s_port);
        i2s_internal_cfg[i2s_port].i2s_state &= (uint8_t)(~(1 << I2S_STATE_TX_RUNNING));
        //log_hal_info("i2s1_state=%d\r\n", i2s1_internal_cfg.i2s_state);
...
void i2s_backup(hal_i2s_port_t i2s_port)
{
    if (i2s_internal_cfg[i2s_port].i2s_state == I2S_STATE_IDLE) 
        return;
}

void i2s_restore(hal_i2s_port_t i2s_port)
{
    if (i2s_internal_cfg[i2s_port].i2s_state == I2S_STATE_IDLE) 
        return;

    //DMA clock on
    if (i2s_internal_cfg[i2s_port].i2s_vfifo.tx_dma_configured || i2s_internal_cfg[i2s_port].i2s_vfifo.rx_dma_configured) {
        DRV_Reg32(FPGA_PDN_CLRD0) |= (uint32_t)0x1;    // DMA clock on
    }
    ...
    if (i2s_port == HAL_I2S_0) {
        ...
        if (i2s_internal_cfg[i2s_port].i2s_vfifo.tx_dma_configured) {
            DRV_Reg32(VDMA7_PGMADDR) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_base;
            DRV_Reg32(VDMA7_COUNT) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_threshold;
            DRV_Reg32(VDMA7_FFSIZE) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_length;
            DRV_Reg32(VDMA7_CON) = (uint32_t)0x10200;
            DRV_Reg32(VDMA7_START) = (uint32_t)0x8000; //enable CH7
        }
        if (i2s_internal_cfg[i2s_port].i2s_vfifo.rx_dma_configured) {
            DRV_Reg32(VDMA8_PGMADDR) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_base;
            DRV_Reg32(VDMA8_COUNT) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_threshold;
            DRV_Reg32(VDMA8_FFSIZE) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_length;
            DRV_Reg32(VDMA8_CON) = (uint32_t)0x10210;
            DRV_Reg32(VDMA8_START) = (uint32_t)0x8000; //enable CH8
        }
        ...
        //VDMA
        if (i2s_internal_cfg[i2s_port].i2s_vfifo.tx_dma_configured) {
            DRV_Reg32(VDMA9_PGMADDR) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_base;
            DRV_Reg32(VDMA9_COUNT) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_threshold;
            DRV_Reg32(VDMA9_FFSIZE) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.tx_vfifo_length;
            DRV_Reg32(VDMA9_CON) = (uint32_t)0x10200;
            DRV_Reg32(VDMA9_START) = (uint32_t)0x8000; //enable CH9
        }
        if (i2s_internal_cfg[i2s_port].i2s_vfifo.rx_dma_configured) {
            DRV_Reg32(VDMA10_PGMADDR) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_base;
            DRV_Reg32(VDMA10_COUNT) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_threshold;
            DRV_Reg32(VDMA10_FFSIZE) = (uint32_t)i2s_internal_cfg[i2s_port].i2s_vfifo.rx_vfifo_length;
            DRV_Reg32(VDMA10_CON) = (uint32_t)0x10210;
            DRV_Reg32(VDMA10_START) = (uint32_t)0x8000; //enable CH10
        }
    }
...
{
    i2s_backup(HAL_I2S_0);
}

void i2s1_enter_suspend(void)
{
    i2s_backup(HAL_I2S_1);
