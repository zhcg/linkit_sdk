...
hal_pinmux_status_t hal_pinmux_set_function(hal_gpio_pin_t gpio_pin, uint8_t function_index)
{
    ...
    gpio_base->GPIO_MODE.CLR[no]= (GPIO_REG_FOUR_BIT_SET_CLR << (remainder * GPIO_MODE_FUNCTION_CTRL_BITS));
    gpio_base->GPIO_MODE.SET[no] = (function_index << (remainder * GPIO_MODE_FUNCTION_CTRL_BITS));
