...
aud_status_code_t audio_file_manager_load_album_cache()
{
    ...
    fres = f_open(&fp, s_file_path, FA_OPEN_EXISTING | FA_READ);
    ...
    fres = f_lseek(&fp, 0);
    if ((fres = f_read(&fp, &album_num, sizeof(uint16_t), &read)) != FR_OK) {
    ...
    if ((fres = f_read(&fp, s_album_state.p_album_item, to_read, &read)) != FR_OK) {
        LOGE("Failed to read album db, fres: %d", fres);
        return AUD_STATUS_CODE_FS_ERROR;
    }
    if (to_read != read) {
...
aud_status_code_t audio_file_manager_save_album_cache()
{
    ...
    fres = f_open(&fp, s_file_path, FA_OPEN_ALWAYS | FA_WRITE | FA_READ);
    if (fres != FR_OK) {
        LOGE("Failed to open album db, fres: %d", fres);
        return AUD_STATUS_CODE_FS_ERROR;
    }
    ...
    do {
        ...
        if ((fres = f_write(&fp, s_album_state.p_album_item, to_write, &written)) != FR_OK) {
            LOGE("Failed to write file, fres: %d", fres);
            ret = AUD_STATUS_CODE_FS_ERROR;
            break;
        }
...
aud_status_code_t audio_file_manager_read_file_db(uint16_t album_idx, uint8_t* buf, uint32_t len, uint32_t offset)
{
    ...
    do {    
        ...
        if ((fres = f_read(&fp, buf, len, &read)) != FR_OK) {
...
int32_t audio_file_manager_create_file_db(TCHAR * folder_name)
{
    ...
    while ((fres = f_readdir(&dir, &fno)) == FR_OK) {
        ...
        fres = f_write(&fp, &rec, sizeof(aud_file_rec_t), &written);
...
bool audio_file_manager_check_db_ready()
{
    ...
    memset(s_file_path, 0x00, MAX_FILE_PATH_LEN);
    ucs2_wsprintf_ex(s_file_path, MAX_FILE_PATH_LEN, _T("%s/%s"), ROOT_PATH, ALBUM_DB);
    fres = f_open(&fp, s_file_path, FA_OPEN_EXISTING);
    if (fres == FR_OK) {
        f_close(&fp);
        return true;
    } else {
...
static FRESULT audio_file_manager_set_breakpoint_file_by_folder(bt_sink_srv_mp3_breakpoint_data_t *bp_data)
{
   ...
   // printf("update bp file\r\n");
    ...
    res = f_open(&fp, breakpoint_path, FA_OPEN_ALWAYS | FA_READ | FA_WRITE);
    if (res == FR_OK) {
        UINT write_p;
        //uint8_t *p;
        res = f_write(&fp, bp_data, sizeof(bt_sink_srv_mp3_breakpoint_data_t), &write_p);
        //p = (uint8_t *)bp_data;
...
static FRESULT audio_file_manager_get_breakpoint_file_by_folder(bt_sink_srv_mp3_breakpoint_data_t *bp_data)
{
    ...
    res = f_open(&fp, breakpoint_path, FA_OPEN_EXISTING | FA_READ | FA_WRITE);
    if (res == FR_OK) {
        res = f_read(&fp, share_buf, AUDIO_FILE_MANAGER_FOLDER_BREAK_POINT_LEN, &read_len);
        ...
        bp_data->offset = 0;
        res = f_open(&fp, breakpoint_path, FA_OPEN_ALWAYS | FA_WRITE);

        if (res == FR_OK) {
            UINT write_p;
            f_write(&fp,(void *)bp_data, sizeof(bt_sink_srv_mp3_breakpoint_data_t), &write_p);
            f_close(&fp);
        }
...
FRESULT audio_file_manager_get_breakpoint(void *data)
{
    ...
    int32_t ret;
    res = f_open(&fp, AUDIO_FILE_MANAGER_BREAKPOINT_CONTROL_PATH, FA_OPEN_EXISTING | FA_READ | FA_WRITE);
    ...
    if (res == FR_OK) {
        ...
        res = f_read(&fp, &temp_folder_id, sizeof(uint16_t), &read_len);
    ...
    if (res != FR_OK) {
        ...
        bp_data->file_id = AUDIO_FILE_MANGER_DEFAULT_FILE_ID;
        bp_data->offset = 0;
        res = f_open(&fp, AUDIO_FILE_MANAGER_BREAKPOINT_CONTROL_PATH, FA_OPEN_ALWAYS | FA_WRITE);
        LOGW("create breakpoint data:%d", res);
        if (res == FR_OK) {
            UINT write_p;
            res = f_write(&fp, (void *) &bp_data->folder_id, 2, &write_p);
            f_close(&fp);
...
#if 0
    FIL fp;
    res = f_open(&fp, AUDIO_FILE_MANAGER_BREAKPOINT_CONTROL_PATH, FA_OPEN_ALWAYS | FA_READ | FA_WRITE);
    UINT write_p;
    if (res == FR_OK) {
        f_write(&fp, (void *)&bp_data->folder_id, 2,  &write_p);
        f_close(&fp);
    } else {
