...
static void gpc_test_hardware_init(hal_gpc_edge_t edge)
{
    ...
    printf("Initialize the GPC GPIO pin, set edge: %d\r\n", edge);
}
...
static void hal_gpc_example(void)
{
    ...
    printf("GPC example test started.\r\n");
    ...
    if (ret != HAL_GPC_STATUS_OK) {
        printf("GPC initialization failed\r\n");
        return;
    }
    ...
    for (uint32_t index = 0; index < 100; index++) {
        ...
        printf("Generated pulse number: %d , read count from GPC: %d\r\n", (int)(index + 1), (int)count_value);
    }
